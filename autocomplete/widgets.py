...
try:
    ADMIN_URL = settings.ADMIN_MEDIA_PREFIX + 'img/admin/'
except AttributeError:
    ADMIN_URL = settings.STATIC_URL + 'admin/img/'

JS_VALUE_MAPPING = {True: 'true', False: 'false', None: 'null'}

def format_js_data(key):
    return ''.join(['-%s' % c.lower() if c.isupper() else c for c in key])
...
#            'js/jquery.min.js',
    ...
    def __init__(self, ac_id, view=default_view, attrs=None, using=None,
                 script_tag=True, **js_options):
