...
// Use when script_tag is set to False.
// This code is transitional, in midterm, the idea is to stop using script tag
// everywhere, but for the moment it doesn't work in admin, partly because the
// admin use jQuery 1.4.2 and version 1.4.3 is needed and for the moment this
// code doesn't support M2M widget.
$(document).ready(function() {
   $('.dj-ac').each(function(elem) {
       ...
       var original = $(this);
       original.attr('id', original.attr('id').replace('id_', 'id_ac_')).removeAttr('name');
       hidden.insertBefore(original);
       original.djangoautocomplete(original.data());
   });
});
